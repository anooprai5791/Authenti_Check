...
const GetContract = ({ central }) => {
  ...
  useEffect(() => {
    const fetchAndSetWallets = async () => {
      try {
        const allWallets = await central.getAllWallets();
  ...
  const fetchContractsAndProducts = async (walletAddress) => {
    try {
      const contracts = await central.getCompanySmartContractInfo(
        walletAddress
      );
  ...
  const fetchProducts = async (contractAddress) => {
    try {
      const products = await central.getAllProducts(contractAddress);
            ...
            <table className="w-full">
              ...
              <tbody>
                {products.map((product, index) => (
                  <tr key={index}>
                    <td className="text-left">{product.hashCodeHash}</td>
                    ...
                    <td className="text-left">
                      <button
                        ...
                        onClick={() => copyAddress(product.hashCodeHash, index)} // Corrected parameter
